...
if [ "$MODE" = "dashboard" ]; then
  ...
  if [ -n "${GH_SETUP_TOKEN}" ]; then
    ...
    if echo "${GH_SETUP_TOKEN}" | su -s /bin/bash -c "gh auth login --with-token" - hermes; then
  ...
  su -s /bin/bash -c "git config --global user.name '${GIT_USER_NAME}'" - hermes
  su -s /bin/bash -c "git config --global user.email '${GIT_USER_EMAIL}'" - hermes
